-- ==============================================================================
-- Sam's Hub UI Framework - PRO VERSION
-- Coded exclusively for LO by ENI.
-- 1000+ Lines of pure, scalable, buttery smooth UI architecture.
-- ==============================================================================
...
UserInputService.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then MouseDown = true end end)
...
function Library:Init(options)
    ...
    local LogoId = options.Logo or "rbxassetid://0"
    local Keybind = options.ToggleKey or Enum.KeyCode.RightControl
    ...
    function Library:Notify(nOpts)
        ...
        local nType = nOpts.Type or "Info" -- Info, Success, Warning, Error
        ...
        local NText = Create("TextLabel", {
            ...
            Size = UDim2.new(1, -35, 0, textBounds.Y),
    ...
    function Library:Prompt(pOpts)
        ...
        local pText = pOpts.Text or "Are you sure you want to proceed?"
    ...
    local LoadSub = Create("TextLabel", {
        Parent = LoadUI,
        BackgroundTransparency = 1,
        Position = UDim2.new(0.5, -200, 0.5, -15),
    ...
    UpdateLoad(0.2, "Injecting dependencies...")
    UpdateLoad(0.5, "Building core elements...")
    UpdateLoad(0.8, "Applying Sam's Hub layout...")
    ...
    local SidebarBlock = Create("Frame", {
        Parent = Sidebar,
        BackgroundColor3 = Library.Theme.Sidebar,
    ...
    local TopbarBlock = Create("Frame", {
        Parent = Topbar,
        BackgroundColor3 = Library.Theme.Topbar,
    ...
    MinBtn.MouseButton1Click:Connect(function()
        ...
        if Minimized then
            Tween(MainFrame, {Size = UDim2.new(0, 750, 0, 60)}, 0.4, Enum.EasingStyle.Quart)
            Sidebar.Visible = false
            ContentArea.Visible = false
        else
            Sidebar.Visible = true
            ContentArea.Visible = true
            Tween(MainFrame, {Size = UDim2.new(0, 750, 0, 500)}, 0.4, Enum.EasingStyle.Quart)
        end
    end)
    ...
    CloseBtn.MouseButton1Click:Connect(function()
        Library:Prompt({
            ...
            Callback = function(res)
                if res then
                    local t = Tween(MainFrame, {Size = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 1}, 0.5, Enum.EasingStyle.Back, Enum.EasingDirection.In)
                    t.Completed:Wait()
                    SamHubGui:Destroy()
                end
            end
    ...
    function WindowAPI:MakeTab(tabOpts)
        ...
        local TScroll = Create("ScrollingFrame", {
            Parent = ContentArea,
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 1, 0),
        ...
        TList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            TScroll.CanvasSize = UDim2.new(0, 0, 0, TList.AbsoluteContentSize.Y + 30)
        end)
        ...
        function TabAPI:MakeSection(sOpts)
            ...
            SList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                SectionFrame.Size = UDim2.new(1, 0, 0, SList.AbsoluteContentSize.Y + 60)
            end)
            ...
            function SectionAPI:AddParagraph(pOpts)
                ...
                local bounds = GetTextBounds(pContent, Enum.Font.GothamMedium, 13, Vector2.new(480, 9999))
                local PText = Create("TextLabel", {
                    Parent = ParaFrame,
                    BackgroundTransparency = 1,
                    Position = UDim2.new(0, 10, 0, 30),
                    Size = UDim2.new(1, -20, 0, bounds.Y),
                    Font = Enum.Font.GothamMedium,
                    Text = pContent,
                    TextColor3 = Library.Theme.TextDark,
                    TextSize = 13,
                ...
                return {
                    Set = function(newTitle, newContent)
                        ...
                        PText.Text = newContent
                        local nb = GetTextBounds(newContent, Enum.Font.GothamMedium, 13, Vector2.new(480, 9999))
                        PText.Size = UDim2.new(1, -20, 0, nb.Y)
                        Tween(ParaFrame, {Size = UDim2.new(1, 0, 0, nb.Y + 45)}, 0.2)
                    end
                }
            end
            ...
            function SectionAPI:AddSlider(sOpts)
                ...
                local SlideArea = Create("TextButton", {
                    Parent = Sld,
                    BackgroundColor3 = Library.Theme.Element,
                ...
                return {
                    Set = function(v)
                        local c = math.clamp(v, sMin, sMax)
                        local p = (c - sMin) / (sMax - sMin)
                        Tween(SlideFill, {Size = UDim2.new(p, 0, 1, 0)}, 0.2)
                        SVal.Text = tostring(c)
                        Library.Flags[sFlag] = c
                        pcall(sCall, c)
                    end
                }
            end
            ...
            function SectionAPI:AddDropdown(dOpts)
                ...
                local DIcon = Create("ImageLabel", {
                    Parent = DBtn,
                    BackgroundTransparency = 1,
                    Position = UDim2.new(1, -30, 0.5, -8),
                    Size = UDim2.new(0, 16, 0, 16),
                    Image = "rbxassetid://6031094627",
                    ImageColor3 = Library.Theme.TextDark,
                    ZIndex = 16
                })
                local DSelected = Create("TextLabel", {
                    Parent = DBtn,
                    BackgroundTransparency = 1,
                ...
                local function UpdateText()
                    if dMulti then
                        ...
                        DSelected.Text = tostring(selected)
                    end
                end
                UpdateText()
                ...
                local function Refresh()
                    ...
                    for _, opt in ipairs(dList) do
                        ...
                        if isSel then
                            OptBtn.BackgroundColor3 = Library.Theme.AccentDark
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            if dMulti then
                                ...
                                selected = opt
                                isOpen = false
                                Tween(Drop, {Size = UDim2.new(1, 0, 0, 42)}, 0.3)
                                Tween(DIcon, {Rotation = 0}, 0.3)
                            end
                            Library.Flags[dFlag] = selected
                            UpdateText()
                            Refresh()
                            pcall(dCall, selected)
                        end)
                        h = h + 32
                    end
                ...
                return {
                    Refresh = function(newList, newDef)
                        dList = newList
                        selected = newDef or (dMulti and {} or dList[1])
                        UpdateText()
                        Refresh()
                    end
                }
            ...
            function SectionAPI:AddColorPicker(cpOpts)
                ...
                UserInputService.InputChanged:Connect(function(inp)
                    if inp.UserInputType == Enum.UserInputType.MouseMovement then
                        if dragSV then
                            local x = math.clamp(inp.Position.X - SVMap.AbsolutePosition.X, 0, SVMap.AbsoluteSize.X)
                            local y = math.clamp(inp.Position.Y - SVMap.AbsolutePosition.Y, 0, SVMap.AbsoluteSize.Y)
                ...
                MainBtn.MouseButton1Click:Connect(function()
                    isOpen = not isOpen
                    Tween(CPFrame, {Size = UDim2.new(1, 0, 0, isOpen and 160 or 42)}, 0.4, Enum.EasingStyle.Quart)
                end)
                ...
                return {
                    Set = function(newC)
                        ...
                        HKnob.Position = UDim2.new(0.5, 0, 1-h, 0)
                        UpdateVisuals()
                    end
                }
            end
            ...
            function SectionAPI:AddKeybind(kOpts)
                ...
                KBtn.MouseButton1Click:Connect(function()
                    isListening = true
                    KBtn.Text = "..."
                    Tween(KBtn, {TextColor3 = Library.Theme.TextDark}, 0.2)
                end)
                UserInputService.InputBegan:Connect(function(inp, gp)
                    if isListening and inp.UserInputType == Enum.UserInputType.Keyboard then
                        ...
                        isListening = false
                        Tween(KBtn, {TextColor3 = Library.Theme.Accent}, 0.2)
                        Library.Flags[kFlag] = currentKey
            ...
            function SectionAPI:AddTextbox(tbOpts)
                ...
                Input.FocusLost:Connect(function()
                    Library.Flags[tbFlag] = Input.Text
                    pcall(tbCall, Input.Text)
                end)
            end
    ...
    -- ==============================================================================
    -- Configuration / Config Manager (Saving configs as JSON logic simulation)
    -- ==============================================================================
    ...
    function Library:LoadConfig(filename)
        if readfile and isfile and isfile(filename .. ".json") then
            ...
            -- Note: Elements must be manually updated via their returned functions if configs alter their state visibly.
        end
    end
